...
public class makeIndexPost {
    ...
    void makeHashMap() throws IOException {
        Integer id=0;
        int count = 0;
        String index_file_path = "SimpleIR/html/index.xml";
        FileOutputStream fileStream = new FileOutputStream("SimpleIR/html/index.post");
        ...
        HashMap <String,String> temp= new HashMap<>();      //값 임시로 담을 temp  해쉬맵
        HashMap <String,String> temp_clone= new HashMap<>();

        HashMap <Integer, Object> id_word = new HashMap<>();   //<id , w(가중치)> result에 넣을 hashmap
        HashMap <String , HashMap<String,String>>result= new HashMap<>();     //<key <id , w(가중치)>> 키워드 id 별 가중치
